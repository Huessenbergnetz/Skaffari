...
FileSizeFormatTag::FileSizeFormatTag(QObject *parent) : Cutelee::AbstractNodeFactory(parent)
{

}

Cutelee::Node *FileSizeFormatTag::getNode(const QString &tagContent, Cutelee::Parser *p) const
{
    QStringList parts = smartSplit(tagContent);
    parts.removeFirst(); // not intereseted in the name of the tag
    if (parts.empty()) {
        throw Cutelee::Exception(Cutelee::TagSyntaxError, QStringLiteral("sk_fsf requires at least the file size"));
    }

    Cutelee::FilterExpression size(parts.at(0), p);

    Cutelee::FilterExpression precision;
    if (parts.size() > 1) {
        precision = Cutelee::FilterExpression(parts.at(1), p);
    }

    Cutelee::FilterExpression base;
    if (parts.size() > 2) {
        base = Cutelee::FilterExpression(parts.at(2), p);
    }

    Cutelee::FilterExpression multiplier;
    if (parts.size() > 3) {
        multiplier = Cutelee::FilterExpression(parts.at(3), p);
    }
...
FileSizeFormat::FileSizeFormat(const Cutelee::FilterExpression &size, const Cutelee::FilterExpression &base, const Cutelee::FilterExpression &precision, const Cutelee::FilterExpression &multiplier, Cutelee::Parser *parser) :
    Cutelee::Node(parser), m_size(size), m_base(base), m_precision(precision), m_multiplier(multiplier)
{

}


void FileSizeFormat::render(Cutelee::OutputStream *stream, Cutelee::Context *gc) const
{
    ...
    if (sizeVarType == qMetaTypeId<Cutelee::SafeString>()) {
        *stream << sizeVar.value<Cutelee::SafeString>().get();
        return;
