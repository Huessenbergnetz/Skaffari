 ...
 * \brief Cutelee template filter to decode ACE encoded strings.
 *
 * This filter can be used as \c sk_acedecode in your Cutelee templates. It decodes
 * a strings that has before been encoded using ASCII Compatible Encoding (ACE).
 *
 * ### Example
 * \code{.html}
 * <!-- will be converted into "hüssenbergnetz.de" if domain contains "xn--hssenbergnetz-wob.de" -->
 * <p>{{ domain|sk_acedecode }}</p>
 * \endcode
 */
class AceDecodeFilter : public Cutelee::Filter // clazy:exclude=copyable-polymorphic
{
public:
    /*!
