...
void AccountEditor::edit(Context* c)
{
    if (Domain::accessGranted(c)) {
        ...
        help.insert(QStringLiteral("password"), HelpEntry(c->translate("AccountEditor", "New password"), c->translate("AccountEditor", "Enter a new password with a minimum length of %n character(s) or leave the field blank to avoid changing the password.", nullptr, SkaffariConfig::accPwMinlength())));
