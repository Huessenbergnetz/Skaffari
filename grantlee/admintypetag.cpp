...
Grantlee::Node *AdminTypeTag::getNode(const QString &tagContent, Grantlee::Parser *p) const
{
    ...
    if (parts.empty()) {
        throw Grantlee::Exception(Grantlee::TagSyntaxError, QStringLiteral("sk_admintypename requires at least the AdminAccount object"));
    }

    Grantlee::FilterExpression admin(parts.at(0), p);

    return new AdminType(admin, p);
}

AdminType::AdminType(const Grantlee::FilterExpression &admin, Grantlee::Parser *parser) :
    Grantlee::Node(parser), m_admin(admin)
{

}

void AdminType::render(Grantlee::OutputStream *stream, Grantlee::Context *gc) const
{
